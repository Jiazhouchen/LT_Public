...
####Data Path: 
#Expecting a string of path that points to the folder with *.csv files in them. 
dt_path <- "/Users/jiazhouchen/Documents/NIH-UCL/RCode/LetterTask_JC/data/LT_SF"

##following section is for loading data
if(!file.exists('LT_SF_analysis.rdata')) {
  ls_csvs <- list.files(path = dt_path,pattern = "*.csv", recursive = F,include.dirs = T,full.names = T)
  ...
  cdf <- rbind(bdf,edf)
  save(sum_df,bdf,edf,cdf,file = "LT_SF_analysis.rdata")
} else {
...
bdf$participant <- as.factor(bdf$uq_index)
...
###Number of each num keys by difficulty, show some pressed "impossible" keys
...
##(10 in Supplement) Early/mid/late portion of the task's effect; to show that later on without reward, the participants are more likely to 
